// Copyright 2026
// Approximate Multiplier C3 for DNN MAC Unit
...
module df_multiplier_c3
	...
	for (i = 2; i <= 6; i = i + 1) begin : genres_approx
		df_approx_fulladder resf_approx(stage3[0][i], stage3[1][i], carrys[i], result[i], carrys[i+1]);
	end
	// EXACT STAGE: Higher significance bits (7-11)
	for (i = 7; i <= 11; i = i + 1) begin : genres_exact
		df_fulladder resf_exact(stage3[0][i], stage3[1][i], carrys[i], result[i], carrys[i+1]);
	end
	endgenerate
